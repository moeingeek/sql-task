...
WITH RECURSIVE cte AS (
   SELECT ch1.id, ch1.name, ch1.manager_id, ch2.manager_id AS next_manager, 2 cntr
   FROM   chart ch1
   LEFT  JOIN chart ch2 ON ch2.id = ch1.manager_id
	WHERE ch1.manager_id IS NOT NULL 
	
   UNION ALL
   SELECT ch1.id, ch1.name, ch1.next_manager, ch2.manager_id, cntr + 1
   FROM   cte ch1
   LEFT   JOIN chart ch2 ON ch2.id = ch1.next_manager
   WHERE  ch1.next_manager IS NOT NULL  -- stop iterating at end of graph
   )	
   

SELECT id, name, cntr as level, manager_id
	FROM   cte
	WHERE  next_manager IS NULL;  -- only rows where the graph ends
